...
# This script fixes the limit_req_zone directive placement
...
# First, add the rate limiting zone to the main nginx.conf if not already there
if ! sudo grep -q "limit_req_zone.*nano_rpc" /etc/nginx/nginx.conf; then
    echo "📝 Adding rate limiting zone to nginx.conf..."
    sudo sed -i '/http {/a\\t# Rate limiting for Nano RPC\n\tlimit_req_zone $binary_remote_addr zone=nano_rpc:10m rate=100r/15m;' /etc/nginx/nginx.conf
fi

# Create the corrected SSL nginx config
...
server {
    ...
    # Proxy to Docker app
    location / {
        # Apply rate limiting (zone defined in nginx.conf)
